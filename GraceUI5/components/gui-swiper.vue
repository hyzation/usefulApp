<template>
	<view class="gui-swiper-card-wrap">
		...
		:indicator-dots="false" :interval="interval" :circular="circular" 
			...
			:key="index" class="gui-swiper-card-item gui-border-box">
				...
				paddingBottom:current != index ? paddingY +'rpx':'0rpx'}">
					<image :style="{
						...
						:src="item.img" :mode="mode" class="gui-swiper-card-image gui-transition-all" />
				</navigator>
				...
				<view @tap.stop="clickBtn" v-if="index == (swiperItems.length - 1) && isShowBtn" class="btn f30 fb">开启生活娱乐</view>
			</swiper-item>
		</swiper>
