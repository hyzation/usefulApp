<template>
	...
	:scroll-left="scrollLeft">
		<view @tap="change_" v-if="leftTitle != ''" class="gui-scroll-x-items gui-columns ml20 gui-bold gui-flex gui-align-items-center gui-nowrap" :style="{marginRight:margin+'rpx',marginTop:'-4rpx'}">
			<text style="font-size:17px;margin-top:-1px;">{{leftTitle1}}</text>
			<text style="font-size:16px;">{{leftTitle}}</text>
		</view>
		...
		:data-index="index">
			...
			:class="[textAlign == 'center' ? 'gui-justify-content-center' : '']">
				...
				:style="{
					...
					fontWeight:currentIndexIn == index ? activeFontWeight : ''}">{{item[keyStr]}}</text>
				<view v-if="item.tips && item.tips != 0">
					<text
...
export default {
	...
	props : {
		...
		keyStr             : {type : String,  default : 'name'},
		leftTitle1         : {type : String,  default : ''},
