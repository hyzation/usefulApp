<template>
	<view :class="[
		...
		<!-- 自定义头部 -->
		<view class="gui-header gui-transition-all" v-if="customHeader" id="guiPageHeader" ref="guiPageHeader" :style="'height:'
		+(headerSets.height+statusBarHeight)+'px; z-index:'
		+headerSets.zIndex+';'+headerStyle">
			<!-- 状态栏 -->
			<view class="gui-page-status-bar" :style="'height:'+statusBarHeight+'px;'+statusBarStyle"></view>
			<!-- 头部插槽 -->
			<view class="gui-flex gui-columns gui-justify-content-center" @tap.stop.prevnet="headerTap"
		...
		<!-- 页面主体 -->
		<view class="gui-flex gui-columns" v-if="!refresh && !loadmore" id="guiPageBody" ref="guiPageBody"
			:class="[fullPage?'gui-flex1':'']">
			<slot name="gBody"></slot>
		</view>
		<!-- 刷新加载主体 -->
		<view class="gui-flex gui-columns gui-flex1" v-if="refresh || loadmore" id="guiPageBody" ref="guiPageBody"
			:style="{
			marginTop:fixedTopMargin+'px', 
			height:refreshBodyHeight+'px'
		}">
			<scroll-view class="gui-relative" :scroll-y="true" :show-scrollbar="false" :style="{
				height:refreshBodyHeight+'px',
				opacity:refreshBodyHeight < 1 ? 0 : 1
			}" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend" @scroll="scroll" :scroll-top="scrollTop"
				@scrolltolower="loadmorefun">
				<view>
					<gui-refresh ref="guiPageRefresh" @reload="reload" :refreshText="refreshText"
						:refreshBgColor="refreshBgColor" :refreshColor="refreshColor"
						:refreshFontSize="refreshFontSize"></gui-refresh>
				</view>
				<slot name="gBody"></slot>
				<view v-if="loadmore" class="gui-page-loadmore">
					<gui-loadmore ref="guipageloadmore" :loadMoreText="loadMoreText" :loadMoreColor="loadMoreColor"
		...
		<!-- 页面底部 -->
		<!-- 底部占位 -->
		<view v-if="customFooter" :style="{height:footerHeight}"></view>
		<view class="gui-page-footer gui-border-box" :class="[isSwitchPage?'gui-switch-page-footer':'']"
			v-if="customFooter" id="guiPageFooter" ref="guiPageFooter" :style="{
			height:footerHeight, 
			...
			<view :style="'height:'+iphoneXButtomHeight+'; '+ iphoneXButtomStyle"></view>
		</view>
		<!-- 右下角悬浮挂件 -->
		<view class="gui-page-pendant" :style="{
		...
		<!-- 吸顶元素 -->
		<view class="gui-page-fixed-top" ref="guiPageFixedTop" id="guiPageFixedTop" :style="{
			top:fixedTop+'px', 
	...
	export default {
		...
		methods: {
			...
			getDomSize: function(domIDOrRef, fun) {
				setTimeout(() => {
					// #ifndef APP-NVUE
					uni.createSelectorQuery().in(this).select('#' + domIDOrRef).boundingClientRect().exec((
						res) => {
							fun(res[0]);
						});
					// #endif
