		...
		<view slot="gBody" class="fm">
			<view class="main gui-flex gui-space-between gui-align-items-center" @tap="showAttr(1)">
				...
				<view class="gui-flex1 ml30">
					<block v-if="address">
						...
						<view class="addressInfob line2 f26 c666666">{{address.province}}{{address.city}}{{address.county}}{{address.street}}{{address.address}}</view>
					</block>
					<block v-else>
						<view class="addressInfou gui-flex gui-align-items-center f28">
			...
			<view @tap="closeAttr" :class="'bgScreen animate '+animateName" v-if="attrShow">
				<view class="bgScreenC gui-absolute-lb" :class="attrType == 1 ? 'bgFFFFFF' : 'bgF5F5F9'" @tap.stop="">
					...
					<view class="attrTitle mainTxt">收货地址</view>
					<block v-if="attrType == 1">
						...
						<navigator hover-class="" url="/pages/ldd/editAddress?id=0" class="sure">添加地址</navigator>
					</block>
				</view>
			</view>
			<!-- 选择支付 -->
...
<script>
	...
	export default {
		...
		onLoad: function(e){
			_this = this;
			_this.postData = uni.getStorageSync('lddPostData');
			uni.removeStorageSync('lddPostData');
			...
			let goods_money = 0;
			let totalMvalue = 0;
			_this.postData.forEach((item)=>{
				sku_list.push({
					sku_sn: item.sku_sn,
					num: item.num,
					price: item.cost_price
				});
				goods_money += item.cost_price * item.num;
				totalMvalue += item.totalMvalue;
			});
			_this.goods_money = goods_money;
			_this.totalMvalue = totalMvalue;
			_this.sku_list = sku_list;
		},
		onReady: function(){
		...
		methods: {
			...
			countFreight: function(){
				_this.freightSuccess = false;
				let area_id = 0;
				if(_this.address.streetId == 0){
					area_id = _this.address.countyId;
				}else{
					area_id = _this.address.streetId;
				}
				let postData = {
					area_id: area_id,
					sku_list: JSON.stringify(_this.sku_list)
				}
				_this.pageLoading = true;
				uni.gRequest.post(
					'swoft/api/ldd/countFreight',
					{
						data: postData
					}, 
					(res) => {
						if(res.code == 0){
							_this.freightSuccess = true;
							_this.freight = Number(res.data).toFixed(2);
							_this.realPrice = (Number(_this.totalPrice)+Number(res.data)).toFixed(2);
						}
					},
					(error) => {
						_this.$refs.guiPage.openMessage('default','请求错误',true,1500);
			...
			submitOrder: function(){
				...
				if(!_this.freightSuccess){
					_this.$refs.guiPage.openMessage('default','获取运费失败,无法下单',true,1500);
					return false;
				}
				let area_id = 0;
				if(_this.address.streetId == 0){
				...
				let postData = {
					...
					sku_list: JSON.stringify(_this.sku_list),
					area_id: area_id,
					area: _this.address.province + _this.address.city + _this.address.county + _this.address.street + _this.address.address,
					name: _this.address.name,
					mobile: _this.address.tel,	
					freight: _this.freight,
					goods_money: _this.goods_money,
					remark: _this.remark,
					payMoney: _this.realPrice,
					totalMvalue: _this.totalMvalue,
