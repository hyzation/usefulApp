				...
				<view class="orderItem" v-for="item in list" :key="item.id">
					...
					<view class="foot">
						...
						<view class="button" v-if="showButton">
							<view class="normal" v-if="item.status == 1" @click="cancel(item)">取消订单</view>
							<view v-if="item.status == 1" class="confirm" @click="gotoPay(item)">去支付</view>
						</view>
						<!-- <view class="error" v-if="!showButton">退款申请中</view> -->
					</view>
				</view>
	...
	export default {
		data() {
			return {
				...
				statusList: ['全部', '待支付', '待发货', '已发货', '', '已完成'],
				currentStatus: '全部',
				currentStatusval: null,
                page: 1,
                pageSize: 10,
				showButton: true,
				topSafe: '',
		...
		methods: {
			...
			async getOrderList (){
                ...
                if(this.currentStatusval) data.status = this.currentStatusval;
                ...
                this.list = getOrderList.data.map(item => {
                    ...
                    switch(item.status) {
                        ...
                        case 2: item.statusText = '待发货'; break;
                        case 3: item.statusText = '已发货'; break;
                        case 4: item.statusText = ''; break;
                        case 5:
			...
			statusChange (status){
				switch(status) {
					case '全部':  this.currentStatusval = 0;break;
					case '待支付':  this.currentStatusval = 1;break;
					case '待发货':  this.currentStatusval = 2;break;
					case '已发货':  this.currentStatusval = 3;break;
					case '已完成':  this.currentStatusval = 5;break;
				}
            ...
            gotoPay (item){
                uni.setStorageSync('readyPayGoods', [{
                    ...
                    cart: item.child.map(goods => {
                        return {
                            product_id: goods.id,
                            product_name: goods.goodsName,
                            price: goods.price,
