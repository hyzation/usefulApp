<template>
	<gui-page :isLoading="pageLoading" :customHeader="true" ref="guiPage">
		...
		<view slot="gBody">
			<view class="card">
				<view class="item">
					<view class="k">产品名称</view>
					<view class="v input">
						<input placeholder-class="phcolor" v-model="name" placeholder="请输入产品名称" />
					</view>
				</view>
				<view class="item">
					<view class="k">产品分类</view>
					<union-selector v-if="drawCategory" :data="categoryList" v-model="category" title="请选择产品分类">
						<view class="v selector">
							<text>{{ category.label || '请选择产品分类' }}</text>
							<uni-icons type="forward" size="20" color="#999999"></uni-icons>
						</view>
					</union-selector>
				</view>
				<view class="item">
					<view class="k">产品图片</view>
					<view class="v">
						<union-upload v-if="drawImage" v-model="image" />
					</view>
				</view>
				<view class="item">
					<view class="k">产品价格</view>
					<view class="v input">
						<input placeholder-class="phcolor" v-model="price" placeholder="请输入产品价格" />
					</view>
				</view>
				<view class="item">
				...
				<view class="item">
					<view class="k">有效期</view>
					<view class="v input">
						<picker mode="date" :value="timeStart"  @change="bindDateChangeStart">
                            <view style="font-size: 24rpx;color: #999999;">{{ datePickerTextStart }}</view>
                        </picker>
                        <text style="font-size: 24rpx;color: #999999;display: inline-block;padding: 0 20rpx;">-</text>
                        <picker mode="date" :value="timeStart"  @change="bindDateChangeEnd">
                            <view style="font-size: 24rpx;color: #999999;">{{ datePickerTextEnd }}</view>
                        </picker>
					</view>
				</view>
			</view>
			<view class="card">
				<view class="cardHead">产品详情</view>
	...
	export default {
		data() {
			return {
                ...
                timeStart: '',
                timeEnd: '',
                datePickerTextStart: '开始日期',
                datePickerTextEnd: '结束日期',
			}
		},
        onLoad (options){
            this.id = options.id;
		...
		async mounted (){
            ...
            that.timeStart = goods.data.info.start_time.substr(0, 10);
            that.datePickerTextStart = goods.data.info.start_time.substr(0, 10);
            that.timeEnd = goods.data.info.end_time.substr(0, 10);
            that.datePickerTextEnd = goods.data.info.end_time.substr(0, 10);
		...
		methods: {
            bindDateChangeStart (e){
                ...
                this.datePickerTextStart = e.detail.value;
            },
            bindDateChangeEnd (e){
                this.timeEnd = e.detail.value;
                this.datePickerTextEnd = e.detail.value;
            },
			goback (){
				uni.navigateTo({
			...
			async submit (){
				...
				let data = {
                    ...
                    price: that.price,
                    stock: that.stock,
                    start_time: that.datePickerTextStart,
                    end_time: that.datePickerTextEnd,
                }
