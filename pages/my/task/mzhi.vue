<template>
	<gui-page :isLoading="pageLoading" ref='guiPage' :customHeader="true">
		<view slot="gHeader">
			<view class="gui-flex gui-nowrap gui-rows gui-align-items-center">
				...
				<view class="gui-flex1 gui-text-center">
					<text class="f36 gui-ellipsis">我的推广值</text>
				</view>
				<view class="pr30">
		...
		<view slot="gBody">
			<view class="bgffc2a6-ffdfd1 gui-border-radius mt20 mlr20 h240">
				<view class="f28 pt40 plr40 fw500" style="display: flex;justify-content: space-between;">
                    <view>本月推广值</view>
                    <view>等级：{{ lev }}</view>
                </view>
				<view class="gui-flex gui-align-items-center gui-space-around mt20">
					<view class="gui-text-center">
						<text class="cFF4F00 f50 fh">{{userInfo.monthMValue}}</text>
						<view class="mt10 f30 fh">我的</view>
					</view>
					<view class="gui-text-center">
						<text class="cFF4F00 f50 fh">{{userInfo.monthTeamMValue}}</text>
						<view class="mt10 f30 fh">社区</view>
					</view>
				</view>
			</view>
			<view class="gui-flex gui-align-items-center gui-space-between plr20 ptb30">
				<view><text class="f34 fb">个人推广值记录</text><text class="f24 c99 ml20">展示最近6个月</text></view>
				...
				<view class="w710 mlr20">
					<view v-for="(item,index) in productList" :key="index" class="list-item gui-flex gui-column gui-justify-content-center gui-relative borderBottom">
						<view class="gui-flex gui-align-items-end gui-space-between">
							<view class="">
								<view class="f28">{{item.from|comeFrom}}<text class="f22 ml20">(订单号：{{item.orderNo}})</text></view>
							...
							<view class="gui-text-right">
								...
								<view class="f30" :class="item.status >= 0 ? 'cFF4F00' : 'c999999'">+{{item.mValue}}</view>
							</view>
						</view>
					</view>
				</view>
			...
			<gui-action-sheet :height="1100" :canCloseByShade="true" listColor="#FF4F00" ref="guiactionsheet" @selected="selected" title="请选择来源" :items="actionSheetItems"></gui-action-sheet>
		</view>
	</gui-page>
</template>
<script>
...
export default {
	...
	filters:{
		comeFrom(e) {
			...
			switch(e){
				...
				case 10: name = '福利购订单'; break;
				case 11: name = '乐惠商城订单'; break;
	...
	onLoad() {
		...
		getApp().readMsgNum(12);
	},
    async mounted (){
        const res = await uni.unionRequest('swoft/api/shop/getpserpv', {
