		...
		<view slot="gHeader">
			<view class="gui-flex gui-nowrap gui-rows gui-align-items-center">
				...
				<view class="gui-flex1 gui-text-center">
					<text class="f36 gui-ellipsis">消费券</text>
				</view>
				<view class="pr30">
		...
		<view slot="gBody">
			<scroll-view id="list" :scroll-y="true" :style="'height:'+mainHeight+'px;'" @scrolltolower="scrolltolower" style="padding: 30rpx 0;">
				...
				<view class="coupon-item">
					...
					<view style="width: 40%;">
						<view class="price">
						{{ couponinfo.dai_ling_qu }}
						</view>
					</view>
					<view class="rightbtn borAll" @click.stop="receiveFun()">立即领取</view>
				</view>
				<view class="coupon-item">
...
export default {
	...
	methods:{
		...
		doSubmit: function() {
			...
			if(_this.numval <= 0 ) {
				_this.$refs.guiPage.openMessage('default','请输入转赠数量',true,1500);
				return false
			}
			if(_this.numval > couponinfo.yu_e ) {
				_this.$refs.guiPage.openMessage('default','转赠数量不能超出余额！',true,1500);
				return false
			}
			console.log(_this.tel)
			console.log(_this.numval)
			uni.gRequest.post(
				'/swoft/api/consumption/send',
				{
					data: {
						user_id: _this.userInfo.id,
						to_user_mobile: _this.tel,
						value: _this.numval
					}
				},
				(res) => {
					if(res.data.code == 20000) {
						_this.getdata()
						_this.$refs.guiPage.openMessage('success','转赠成功！',true,1500);
					} else {
						_this.$refs.guiPage.openMessage('default','暂无可领取的消费积分',true,1500);
					}
