				...
				<view class="w710 mlr20">
					<view v-for="(item,index) in productList" :key="index" class="list-item gui-flex gui-column gui-justify-content-center gui-relative borderBottom">
						<view class="gui-flex gui-align-items-center gui-space-between">
							<view class="">
								<view class="" v-if="item.type == 1"><text class="f28">{{item.bankName}}</text> <text class="f22 ml10">({{item.bankCard|dealBankCardNumber}})</text></view>
								<view class="" v-if="item.type == 2"><text class="f28">支付宝</text> <text class="f22 ml10">({{item.aliAccount|dealPhoneNumber}})</text></view>
								<view class=""><text class="f22">订单号：{{item.orderNo}}</text></view>
								<view class="f22 c99 mt10">{{item.created_at}}</view>
							</view>
							<view class="gui-text-right">
								<block v-if="item.status == -1">
									<view class="c999999 f28">提现失败</view>
									<view class="c999999 f28 mt10">-{{item.money}}</view>
								</block>
								<block v-if="item.status == 0">
									<view class="cFD2C55 f28">提现中</view>
									<view class="cFD2C55 f28 mt10">-{{item.money}}</view>
								</block>
								<block v-if="item.status == 1">
									<view class="f28 cFF4F00">提现成功</view>
									<view class="cFD2C55 f28 mt10">-{{item.money}}</view>
								</block>
							</view>
						</view>
