	...
	<gui-page :isLoading="pageLoading" :apiLoadingStatus="apiLoadingStatus" :loadmore="true" @loadmorefun="getdata" ref="guiPage">
		...
		<view slot="gBody" class="fm">
			...
			<view class="padding_30_" v-for="(item,index) in productList" :key="index">
				<navigator hover-class="" :url="'/pages/my/order/detail?id='+item.id" class="bg-FFFFFF border-r20">
					<view class="p20 gui-relative borderBottom gui-flex gui-space-between gui-align-items-center" v-for="(item2,index2) in item.child" :key="index2">
						<view class="view4 border-r10">
							<image :src="item2.coverPath+'?imageslim'" mode="aspectFill"></image>
						</view>
						<view class="view5">
							<block v-if="item2.activityType == 0">
								<view class="gui-ellipsis fontsize28">{{item2.goodsName}}</view>
							</block>
							<block v-if="item2.activityType == 1">
								<view class="gui-flex gui-align-items-center">
									<view class="sbtn bgFF4F00">0元购</view>
									<view class="gui-ellipsis fontsize28 width-r">{{item2.goodsName}}</view>
								</view>
							</block>
							<block v-if="item2.activityType == 2">
								<view class="gui-flex gui-align-items-center">
									<view class="sbtn bgFF4F00">9.9包邮</view>
									<view class="gui-ellipsis fontsize28 width-r">{{item2.goodsName}}</view>
								</view>
							</block>
							<view class="f24 color-999999 margin-t20 gui-ellipsis">{{item2.goodsOptionName}}</view>
							<view class="color-999999 fontsize24 gui-flex gui-align-items-end gui-space-between margin-t20">
								<block v-if="item2.status == 4">
									<view class="gui-flex gui-space-between w300 gui-align-items-center">
										<text class="color-FF0D00 fontsize30">￥{{item2.price}}</text>
										<text class="f24 color-999999">x{{item2.num}}</text>
									</view>
									<navigator @tap.stop="" :url="'/pages/my/order/postComment?orderId='+item.id+'&orderGoodsId='+item2.id+'&goodsId='+item2.goodsId" hover-class="" class="orderBtn_ bgFF4F00">去评价</navigator>
								</block>
								<block v-else>
									<text class="color-FF0D00 fontsize30">￥{{item2.price}}</text>
									<text class="f24 color-999999">X{{item2.num}}</text>
							...
							<view v-if="item2.status == -5" class="fontsize24 gui-flex gui-align-items-end gui-justify-content-end margin-t20">
								<view class="orderBtn2_">退货中</view>
							</view>
							<view v-if="item2.status == -6" class="fontsize24 gui-flex gui-align-items-end gui-justify-content-end margin-t20">
					...
					<view class="padding_lr30 padding_b30 gui-relative borderBottom">
						<view class="f26 padding_t20 gui-flex gui-align-items-center gui-space-between">
							...
							<text v-if="item.status == 4" class="cFF4F00">待评价</text>
							<text v-if="item.status == 5" class="cFF4F00">已收货</text>
							<text v-if="item.status == 6" class="c999999">已完成</text>
							<text v-if="item.status == -3" class="cFF4F00">退款中</text>
							<text v-if="item.status == -4" class="c999999">已退款</text>
							<text v-if="item.status == -5" class="cFF4F00">退货中</text>
							<text v-if="item.status == -6" class="c999999">已退货</text>
						</view>
						<view class="f26 padding_t20 gui-flex gui-align-items-center gui-space-between">
							...
							<view class="gui-flex gui-align-items-center gui-justify-content-end">
								...
								<text class="f24" v-if="item.payType == 1">微信支付</text>
								<text class="f24" v-if="item.payType == 2">支付宝支付</text>
								<text class="f24" v-if="item.payType == 3">现金券支付</text>
								<text class="f24" v-if="item.payType == 4">余额支付</text>
							</view>
						</view>
					</view>
					<view class="padding_30 gui-flex gui-align-items-center gui-space-between">
						<view>
							...
							<text class="f26 margin-l20">
								<text>总计</text>
								<text class="color-FF0D00">￥{{item.realPrice}}</text>
							</text>
						</view>
						<view class="gui-flex gui-align-items-center">
							...
							<block v-if="item.status == 1">
								...
								<view class="orderBtn bgFF4F00" @tap.stop="payOrder(item.id,item.payType,item.realPrice)">立即付款</view>
							</block>
	...
	export default {
		...
		onLoad: function(e){
			...
			if(_this.status == 99){
				...
				getApp().readMsgNum(_this.status+1);
			}
		},
		onShow: function(){
			_this.reload();
