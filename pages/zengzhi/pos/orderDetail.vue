<template>
	<gui-page :isLoading="pageLoading" :customHeader="false" class="bg-F8F8F8" ref="guiPage">
		...
		<view slot="gBody" class="fm">
			<view class="padding_30" v-if="data">
				...
				<view class="bg-FFFFFF border-r20 pl30 pr30 pb30">
					...
					<view class="fontsize24 pt20 cFF4F00" v-if="data.status == 4">已完成</view>
					<view v-if="data.child" class="pb30 pt20 gui-flex gui-space-between gui-align-items-center gui-relative borderBottom">
						<view class="view4 border-r10">
							<image :src="data.child.coverPath+'?imageslim'" mode="aspectFill"></image>
						</view>
						<view class="view5">
							<view class="gui-ellipsis fontsize28">{{data.child.title}}</view>
							<view class="fontsize22 color-999999 margin-t20 gui-ellipsis">{{data.child.desc}}</view>
							<view class="color-999999 fontsize24 gui-flex gui-align-items-end gui-space-between margin-t20">
								<text class="cFF4F00 fontsize30">￥{{(Number(data.totalPrice)/data.num).toFixed(2)}}</text>
								<text class="fontsize22 color-999999">x{{data.num}}</text>
							</view>
						</view>
					</view>
					<view >
						...
						<view class="fontsize28 padding_t20 gui-flex gui-align-items-center gui-space-between" v-if="data.sendTime != '1970-01-01 08:00:00'">
							<text style="max-width:550rpx;">物流信息：{{data.ex_name}} {{data.expressNo}}</text>
							<text class="cFF4F00 f24" @tap="copy(data.expressNo)">复制</text>
						</view>
				...
				<view class="bg-FFFFFF padding_30 margin-t30 border-r20">
					<view>
						...
						<text class="cFF4F00 float-r fontsize26">￥{{data.totalPrice}}</text>
					</view>
					<view class="margin-t30">
						<text class="fontsize28">运费</text>
						<text class="cFF4F00 float-r fontsize26">到付</text>
					</view>
					<view class="margin-t30" v-if="data.payMoney > 0 && data.status > 1">
						<text class="fontsize28">已支付</text>
						<text class="cFF4F00 float-r fontsize26">￥{{data.payMoney}}</text>
					</view>
					<view class="text-right fontsize24 padding_t30" v-if="data.realPrice > 0 && data.status == 1">
						<text>需付款：</text>
						<text class="cFF4F00">￥{{data.totalPrice}}</text>
					</view>
				</view>
			</view>
			...
			<view class="padding30 gui-fixed-lb gui-flex gui-justify-content-end gui-align-items-center">
				...
				<block v-if="data.status == 1">
					...
					<view class="orderBtn bgFF4F00" @tap.stop="payOrder(data.id,data.payType,data.totalPrice)">立即付款</view>
				</block>
	...
	export default {
		...
		methods: {
			...
			getOrderDetail: function(){
				...
				uni.gRequest.post(
					'swoft/api/pos/getPosOrderDetail',
					{
						data: {
			...
			changeOrderStatus: function(){
				let url = '';
				let postData = {};
					url = 'swoft/api/pos/editPosOrder';
					postData = {
						uid: uni.getStorageSync('userInfo').id,
			...
			payOrder: function(id,payType,totalPrice){
				...
				postData.type = 'H5';
				if(_this.payType == 1){
					postData.return_url = 'https://useful.youyong.org.cn/H5/#/pages/zengzhi/pos/orderDetail?id='+_this.id;
				}else{
					postData.return_url = '';
				}
				//#endif
