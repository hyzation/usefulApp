<template>
	<gui-page :isLoading="pageLoading" ref='guiPage'>
		<view slot="gBody">
			<view class="w710 mlr20 mt20 pb30 gui-relative borderBottom">
				<view class="gui-flex gui-align-items-center gui-space-between">
					<view class=""><text class="f34 fb">充值记录</text><text class="f24 c99 ml20">展示最近6个月</text></view>
			...
			<scroll-view id="list" :scroll-y="true" :style="'height:'+mainHeight+'px;'" @scrolltolower="scrolltolower">
				<view class="w710 mlr20">
					<view v-for="(item,index) in productList" :key="index" class="list-item gui-flex gui-column gui-justify-content-center gui-relative borderBottom">
						<view class="gui-flex gui-align-items-center gui-space-between">
							<view class="">
								<view class=""><text class="f28">{{item.type == 1 ? '话费' : '电费'}}充值</text> <text class="f22">(订单号：{{item.outTradeNum}})</text></view>
								<view class="">
									<text class="f22">{{item.type == 1 ? item.gsd : item.unit + ' ' + item.area + ' ' + item.city}}</text>
									<text class="f22 ml20">{{item.mobile}}</text>
								</view>
								<view class="f22 c99 mt20">{{item.time}}</view>
							</view>
							<view class="gui-text-right">
								<block v-if="item.state == -1">
									<view class="c999999 f28">充值失败</view>
									<view class="c999999 f28 mt10">{{item.amount}}元</view>
								</block>
								<block v-if="item.state == 0">
									<view class="cFD2C55 f28">充值中</view>
									<view class="cFD2C55 f28 mt10">{{item.amount}}元</view>
								</block>
								<block v-if="item.state == 1">
									<view class="f28 cFF4F00">充值成功</view>
									<view class="f28 mt10 cFF4F00">{{item.amount}}元</view>
								</block>
							</view>
						</view>
					</view>
			...
			<gui-action-sheet :height="530" :canCloseByShade="true" listColor="#FF4F00" ref="guiactionsheet" @selected="selected" title="请选择类别" :items="actionSheetItems"></gui-action-sheet>
		</view>
	</gui-page>
</template>
<script>
...
export default {
	...
	methods:{
		selected: function(index){
			_this.state = _this.actionSheetItems[index].id;
			_this.reload();
		},
